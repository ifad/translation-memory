...
class Txml < Element
  def inspect
    "#<Txml concept_groups: #{concept_groups.size}>"
  end
...
class ConceptGroup < Element
  def inspect
    "#<ConceptGroup languages: #{languages} groups: #{language_groups.size}>"
  end
...
class LanguageGroup < Element
  def inspect
    "#<LanguageGroup language: #{language} terms: #{term_groups.size}>"
  end
...
class TermGroup < Element
  ...
  def created_at
    text('./transacGrp/transac[@type="origination"]/following-sibling::date')
  end
  ...
  def updated_at
    text('./transacGrp/transac[@type="modification"]/following-sibling::date')
  end
end
